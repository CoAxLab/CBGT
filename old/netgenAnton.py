...
def describeBG(**kwargs):
    ...
    D1STR = makePop("D1STR", [GABA, [AMPA, 800, 4., 1.29], NMDA], cd_pre)
    camP(c, 'D1STR', 'D1STR', 'GABA', ['syn'], .175, .28)
    camP(c, 'D1STR', 'D2STR', 'GABA', ['syn'], .175, .28)
    camP(c, 'D1STR', 'GPi', 'GABA', ['syn'], .58, 1.0, name='direct')

    D2STR = makePop("D2STR", [GABA, [AMPA, 800, 4., 1.29], NMDA], cd_pre)
    ...
    camP(c, 'FSI', 'D1STR', 'GABA', ['all'], .7, 1.215)
    camP(c, 'FSI', 'D2STR', 'GABA', ['all'], .66, 1.215)
...
def describeBG2(**kwargs):
    ...
    FSI = makePop("FSI", [GABA, [AMPA, 800, 1.5, 3.2], NMDA], cd_pre, {'C': 0.2, 'Taum': 10})
    camP(c, 'FSI', 'FSI', 'GABA', ['all'], .85, 1.15)
    camP(c, 'FSI', 'D1STR', 'GABA', ['all'], .7, 1.225)
    camP(c, 'FSI', 'D2STR', 'GABA', ['all'], .66, 1.225)

    # GPeP = makePop("GPeP", [[GABA, 2000, 2, 2], [AMPA, 800, 2, 5], NMDA],
                    # cd_pre, {'N': 2000, 'tauhm': 10, 'g_T': 0.01})
    GPeP = makePop("GPeP", [[GABA, 2000, 3, 2], [AMPA, 800, 2, 5], NMDA],
                    cd_pre, {'N': 2000, 'g_T': 0.06})
    camP(c, 'GPeP', 'GPeP', 'GABA', ['all'], 0.02, 1.5)
    camP(c, 'GPeP', 'STNE', 'GABA', ['syn'], 0.02, 0.4)
    ...
    camP(c, 'STNE', 'GPi', 'NMDA', ['all'], 1, 0.0315)
    GPi = makePop("GPi", [ GABA, [AMPA, 800, config['GPiExtEff'], 0.8], NMDA], cd_pre)
    camP(c, 'GPi', 'Th', 'GABA', ['syn'], .85, 0.05)

    Th = makePop('Th', [GABA, [AMPA, 800, 25, 20], NMDA], cd_pre)
    camP(c, 'Th', 'D1STR', 'AMPA', ['all'], 0.35, config['ThSTR'])
    camP(c, 'Th', 'D2STR', 'AMPA', ['all'], 0.35, config['ThSTR'])
    camP(c, 'Th', 'FSI', 'AMPA', ['all'], 0.15, config['ThSTR']/1.5)
    camP(c, 'Th', 'LIP', 'NMDA', ['all'], 0.25, config['ThCx'], name='thcx')
    ...
    if config['rampingCTX']:
        camP(c, 'LIP', 'LIP', ['AMPA', 'NMDA'], ['all'], .125, [0.018, 0.15])
