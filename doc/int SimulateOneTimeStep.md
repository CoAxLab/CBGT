Maintains an array of new external frequencies (freq[MAXP][MAXRECEPTORS]) and last external frequencies (last_freq[MAXP][MAXRECEPTORS]) for each population-receptor pair.  The last frequency is used to determine the new external frequency in the case where the external frequency standard deviation is non-zero, otherwise the new external frequency is just its mean value.  On the very first timestep, all last frequencies are initialized to the mean external frequencies.
    ...
    For each population
        For each receptor type
            If external frequency standard deviation is non-zero
                Calculate the new external frequency.  Using the mean and standard deviation, first randomly picks a "target" external frequency, and then moves the last frequency 1-.FreqExtDecay proportion of the way (0.001 by default) towards the target.
                Mathematical note: this means the long-term standard deviation of the external input is actually FESD*sqrt((1-FED)/(FED+1))
            Else if standard deviation is zero
                New external frequency is just the mean external frequency
        For each cell
            For each receptor type
                Calculate .ExtMuS and .ExtSigmaS using .MeanExtEff, corresponding to EQ 5,6 but for external inputs
                Update .LS to represent the decay of total internal conductances (EQ 5,6 term 2 for internal inputs)
    For each population
        For each delayed spiking neuron in that population (the neurons whose spikes are now arriving)
            For each synapse of the spiking neuron
                Update short-term depression conductance multiplier value (EQ 7 term 2)
                Update short-term facilitation conductance multiplier value (EQ 7 term 2)
                If .LastConductance < 0
                    Update the total internal conductance (.LS) of the target neuron according the STD, STF, and efficacy. ()
                Else
                    Decay the LastConducatance of the synapse (meaning increased potency?)
                    Update the total internal conductance (.LS) of the target neuron according the STD, STF, efficacy, and 1-LastConductance of the synapse (EQ 5,6 term 1)
                    Add a boost to the LastConductance of the synapse due to the spike (bringing it closer to 1)
                Add boosts to STD and STF due to the spike (EQ 7 term 1)
    For each population
        Reset number of emitted spikes in buffer to 0
        For each cell
            If potential (V) is less than V_h
                Adjust inactivation variable of the current (.h) according to equation 3 (not exact calculation)
                Coefficient of second term of EQ 1 (g_rb, rebound burst) is zero (Heaviside function)
            Else
                Adjust .h according to equation 2 (not exact calculation)
                Coefficient of second term of EQ 1 is .g_T * .h
            If cell potential is greater than cell threshold, set potential to the reset potential (.ResetPot, -55 mV in example configurations, slightly less than threshold at -55 mV), and decrement .RefrState (cooldown timer for refraction).  Move on to the next cell.
            If cell is in refractory period, decrement and move on to the next cell.
            Calculate the anomalous delayed rectifier (ADR, g_adr)
            If cell has no potassium outward rectifying current (.g_k_max == 0)
                Potassium conductance is zero (g_k = 0)
            Else
                Update outward potassium channel gating variable (.n_k), and then g_k = .g_k_max * .n_k
            If efficacy for calcium-activated potassium channels (.g_ahp) is zero
                Calculate change in cell potential.  Terms include restoration to resting potential (EQ 1 term 1), contribution from anomalous delayed rectifier (not shown in paper), contribution from outward potassium current (not shown in paper), and T-type calcium current (EQ 1 term 2).
            Else
                Calculate change in cell potential using an extra term.  Terms include restoration to resting potential (EQ 1 term 1), contribution from calcium-activated potassium channels, contribution from anomalous delayed rectifier (not shown in paper), contribution from outward potassium current (not shown in paper), and T-type calcium current (EQ 1 term 2).
            Decay calcium concentrations (first order approximation)
            Create Vaux auxillary variable, which is cell potential truncated to the threshold voltage, used in some spots for numerical accuracy
            For each receptor type, calculate contribution to cell potential using both internal (.LS) and external (.ExtS) conductances. (EQ 1 term 3)
                If .MgFlag is true, which is only set for NMDA, then use a more complex term (EQ 4 term 2)
                Otherwise, use a simpler expression (EQ 4 terms 1,3)
            If cell potential greater than threshold, a spike is emitted
                Insert this cell ID into the .TableofSpikes, and increment .NTableofSpikes
                (Temporarily reset cell potential, not really though, the actual reset occurs next time step)
                Update .PTimeLastSpike and .TimeLastSpike for the neuron
                Set cell potential to 0, as the spike
                Set refractory period cooldown timer .RefrState = (int)floor(2. / dt), so refractory period is 2 ms
                Bump calcium concetration due to spike
    For each population
        Update pointers for circular spike buffers
