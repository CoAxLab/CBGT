...
iterations=12;
for iter=10:iterations
    Config='Kyle';
    %Config='Jon';
    
    r1Prob = 0.65;
    r2Prob = 0.35;
    fileName='./Dopamine_p1_065';
    disp(iter);
    % ::::::::::::::::::: PARAMETERS :::::::::::::::::::
    % Network parameters.
    ...
    tf=15000;              % Final time of the simulation
    ...
    c2=2.5;                                   % To control tonic & phasic DA in D2 neurons.
    
    wmax_D1=0.13; wmin_D1=0;  % max and min values for w w'=alpha*E*DA*abs(wi-w), where i=max or min
    ...
    action1Thr=600;       % Number of actions 1 necesary to switch the reward
    action2Thr=600;       % Number of actions 2 necesary to switch the reward
    ...
    for i=nD1act1+nD1act2+1:nNeurons  % D2-cells
        ...
        x0dop(pos+2)=0;       % APOST = Post-synaptic firing rate
        x0dop(pos+3)=0;       % E = Eligibility
        x0dop(pos+4)=0.018;    % w = Strenght        %0.009;
    end
