    ...
    if (PopD[p].dpmn_type) {
      for (i = 0; i < Pop[p].Ncells; i++) {
        ...
        if (Pop[p].Cell[i].dpmn_type) {
          ...
          Pop[p].Cell[i].dpmn_w += dt * Pop[p].Cell[i].dpmn_alphaw * Pop[p].Cell[i].dpmn_E * fDA
                        * (Pop[p].Cell[i].dpmn_wmax - Pop[p].Cell[i].dpmn_w);
        }
      }
    }
  }
  ...
  for (p = 0; p < Npop; p++) {
    ...
    if ((counter % STEPSFORSAVINGFREQS) == 0) {
      fprintf(devfreqs, "%f\t",
            ...
            //  Pop[p].Cell[0].dpmn_DAt + Pop[p].Cell[1].dpmn_DAp + Pop[p].Cell[2].dpmn_DAt + Pop[p].Cell[3].dpmn_DAp );  // debugging dopamine
    }
