...
def describeBG(**kwargs):
    ...
    STNE = makePop("STNE", [GABA, [AMPA, 800, config['STNExtEff'], 4], NMDA], cd_pre,
                   {'N': 2500, 'g_T': 0.06})
    camP(c, 'STNE', 'GPe', ['AMPA', 'NMDA'], ['syn'], 0.05, [0.05, 2])
    camP(c, 'STNE', 'GPi', 'NMDA', ['all'], 1, 0.04)
    GPe = makePop("GPe", [[GABA, 2000, 2, 2], [AMPA, 800, 2, 4], NMDA], cd_pre,
                  {'N': 2500, 'tauhm': 10, 'g_T': 0.01})
    camP(c, 'GPe', 'GPe', 'GABA', ['all'], 0.02, 1.25)
    camP(c, 'GPe', 'STNE', 'GABA', ['syn'], 0.02, 0.8)
    camP(c, 'GPe', 'GPi', 'GABA', ['syn'], 1, 0.05)
    camP(c, 'GPe', 'D1STR', 'GABA', ['all'], 1, 0.01, name='arkipallidal')
    camP(c, 'GPe', 'D2STR', 'GABA', ['all'], 1, 0.01, name='arkipallidal')
    D1STR = makePop("D1STR", [GABA, [AMPA, 800, 4, 1.4], NMDA], cd_pre)
    camP(c, 'D1STR', 'D1STR', 'GABA', ['syn'], 1, 1.)
    camP(c, 'D1STR', 'GPi', 'GABA', ['syn'], 1, 2.7, name='direct')
    D2STR = makePop("D2STR", [GABA, [AMPA, 800, 4, 1.4], NMDA], cd_pre)
    camP(c, 'D2STR', 'D2STR', 'GABA', ['syn'], 1, 1.)
    camP(c, 'D2STR', 'GPe', 'GABA', ['syn'], 1, 2.7, name='indirect')
    LIP = makePop("LIP", [GABA, [AMPA, 800, 2.0, 2.8],
                          NMDA], cd_pre, {'N': 240})
    camP(c, 'LIP', 'Th', 'AMPA', ['syn'], 1, 0)
    camP(c, 'LIP', 'D1STR', 'AMPA', ['syn'], 1, config['CxSTR'], name='cxd')
    camP(c, 'LIP', 'D2STR', 'AMPA', ['syn'], 1, config['CxSTR'], name='cxi')
    # camP(c, 'LIP', 'D1STR', 'AMPA', ['syn'], 1, 1.0)
    # camP(c, 'LIP', 'D2STR', 'AMPA', ['syn'], 1, 1.0)

    # camP(c, 'LIP', 'LIPb', ['AMPA', 'NMDA'], ['all'], 1., [0.05, 0.165])
    camP(c, 'LIP', 'LIP', ['AMPA', 'NMDA'], ['syn'], 1, [0.08, 0.2805])
    # camP(c, 'LIP', 'LIP', ['AMPA', 'NMDA'], ['anti'], 1, [0.043825, 0.14462])
    camP(c, 'LIP', 'LIPI', ['AMPA', 'NMDA'], ['all'], 1., [0.04, 0.13])
    Th = makePop('Th', [GABA, [AMPA, 800, 2, 3.2], NMDA], cd_pre)
    # camP(c, 'Th', 'Th', 'NMDA', ['syn'], 1, 1.5)
    ...
    # camP(c, 'LIPb', 'LIP', ['AMPA', 'NMDA'], ['all'], 1, [0.043825, 0.14462])
    # camP(c, 'LIPb', 'LIPI', ['AMPA', 'NMDA'], ['all'], 1, [0.04, 0.13])
    LIPI = makePop("LIPI", [GABA, [AMPA, 800, 1.15, 3], NMDA], cd_pre, { 'N': 300, 'C': 0.2, 'Taum': 10})
    # camP(c, 'LIPI', 'LIPb', 'GABA', ['all'], 1, 1.35)
    camP(c, 'LIPI', 'LIP', 'GABA', ['all'], 1, 1.12)
    camP(c, 'LIPI', 'LIPI', 'GABA', ['all'], 1, 1.075)
    # brain = makeChannel('brain', [LIPb, LIPI], [action_channel])
    brain = makeChannel('brain', [LIPI], [action_channel])
